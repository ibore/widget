...
public class DayWheelView extends WheelView<Integer> {
    ...
    public DayWheelView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mYear = typedArray.getInt(R.styleable.DayWheelView_dwvYear, mCalendar.get(Calendar.YEAR));
        mMonth = typedArray.getInt(R.styleable.DayWheelView_dwvMonth, mCalendar.get(Calendar.MONTH) + 1);
        int selectedDay = typedArray.getInt(R.styleable.DayWheelView_dwvSelectedDay, mCalendar.get(Calendar.DATE));
        typedArray.recycle();
        updateDay();
