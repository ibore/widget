...
public class RadarProgressView extends View {
    //默认的主题颜色
    ...
    public RadarProgressView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        getAttrs(context, attrs);
        init();
    }

    public RadarProgressView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            TypedArray mTypedArray = context.obtainStyledAttributes(attrs, R.styleable.RadarProgressView);
            mCircleColor = mTypedArray.getColor(R.styleable.RadarProgressView_rpvCircleColor, DEFAULT_COLOR);
            mCircleNum = mTypedArray.getInt(R.styleable.RadarProgressView_rpvCircleNum, mCircleNum);
            if (mCircleNum < 0) {
                mCircleNum = 3;
            }
            mSweepColor = mTypedArray.getColor(R.styleable.RadarProgressView_rpvSweepColor, DEFAULT_COLOR);
            mRaindropColor = mTypedArray.getColor(R.styleable.RadarProgressView_rpvRaindropColor, DEFAULT_COLOR);
            mRaindropNum = mTypedArray.getInt(R.styleable.RadarProgressView_rpvRaindropNum, mRaindropNum);
            isShowCross = mTypedArray.getBoolean(R.styleable.RadarProgressView_rpvShowCross, true);
            isShowRaindrop = mTypedArray.getBoolean(R.styleable.RadarProgressView_rpvShowRaindrop, true);
            mSpeed = mTypedArray.getFloat(R.styleable.RadarProgressView_rpvSweepColor, mSpeed);
            if (mSpeed <= 0) {
                mSpeed = 3;
            }
            mFlicker = mTypedArray.getFloat(R.styleable.RadarProgressView_rpvFlicker, mFlicker);
            if (mFlicker <= 0) {
                mFlicker = 3;
            }
            mTypedArray.recycle();
    ...
    private void drawCircle(Canvas canvas, int cx, int cy, int radius) {
        ...
        for (int i = 0; i < mCircleNum; i++) {
            canvas.drawCircle(cx, cy, radius - (radius / mCircleNum * i) - mCirclePaint.getStrokeWidth() / 2, mCirclePaint);
        }
    }
    ...
    private static class Raindrop {
        ...
        public int changeAlpha() {
            return RadarProgressView.changeAlpha(color, (int) alpha);
        }
