...
public class CircleProgressView extends ProgressBar {
    ...
    private void obtainAttributes(AttributeSet attrs) {
        ...
        mProgressStyle = ta.getInt(R.styleable.CircleProgressView_cpvStyle, ProgressStyle.NORMAL);
        // 获取三种风格通用的属性
        mNormalBarSize = (int) ta.getDimension(R.styleable.CircleProgressView_cpvNormalSize, mNormalBarSize);
        mNormalBarColor = ta.getColor(R.styleable.CircleProgressView_cpvNormalColor, mNormalBarColor);

        mReachBarSize = (int) ta.getDimension(R.styleable.CircleProgressView_cpvReachSize, mReachBarSize);
        mReachBarColor = ta.getColor(R.styleable.CircleProgressView_cpvReachColor, mReachBarColor);

        mTextSize = (int) ta.getDimension(R.styleable.CircleProgressView_cpvTextSize, mTextSize);
        mTextColor = ta.getColor(R.styleable.CircleProgressView_cpvTextColor, mTextColor);
        mTextSkewX = ta.getDimension(R.styleable.CircleProgressView_cpvTextSkewX, 0);
        if (ta.hasValue(R.styleable.CircleProgressView_cpvTextSuffix)) {
            mTextSuffix = ta.getString(R.styleable.CircleProgressView_cpvTextSuffix);
        }
        if (ta.hasValue(R.styleable.CircleProgressView_cpvTextPrefix)) {
            mTextPrefix = ta.getString(R.styleable.CircleProgressView_cpvTextPrefix);
        }
        mTextVisible = ta.getBoolean(R.styleable.CircleProgressView_cpvTextVisible, mTextVisible);

        mRadius = (int) ta.getDimension(R.styleable.CircleProgressView_cpvRadius, mRadius);
        ...
        switch (mProgressStyle) {
            ...
            case ProgressStyle.FILL_IN_ARC:
                mStartArc = ta.getInt(R.styleable.CircleProgressView_cpvStartArc, 0) + 270;
                mInnerPadding = (int) ta.getDimension(R.styleable.CircleProgressView_cpvInnerPadding, mInnerPadding);
                mOuterColor = ta.getColor(R.styleable.CircleProgressView_cpvOuterColor, mReachBarColor);
                mOuterSize = (int) ta.getDimension(R.styleable.CircleProgressView_cpvOuterSize, mOuterSize);
                mReachBarSize = 0;// 将画笔大小重置为0
                mNormalBarSize = 0;
                if (!ta.hasValue(R.styleable.CircleProgressView_cpvNormalColor)) {
            ...
            case ProgressStyle.NORMAL:
                mReachCapRound = ta.getBoolean(R.styleable.CircleProgressView_cpvReachCapRound, true);
                mStartArc = ta.getInt(R.styleable.CircleProgressView_cpvStartArc, 0) + 270;
                if (ta.hasValue(R.styleable.CircleProgressView_cpvInnerBackgroundColor)) {
                    mInnerBackgroundColor = ta.getColor(R.styleable.CircleProgressView_cpvInnerBackgroundColor, Color.argb(0, 0, 0, 0));
