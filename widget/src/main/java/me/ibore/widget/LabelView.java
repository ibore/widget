...
public class LabelView extends View {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mFqSize = ta.getDimension(R.styleable.LabelView_lvFillSize, 0f);
        ...
        mTextSize = ta.getDimension(R.styleable.LabelView_lvTextSize, sp2px(11));
        ...
        mMinSize = ta.getDimension(R.styleable.LabelView_lvMinSize, mFillTriangle ? dp2px(35) : dp2px(50));
        mPadding = ta.getDimension(R.styleable.LabelView_lvPadding, dp2px(3.5f));
        mGravity = ta.getInt(R.styleable.LabelView_lvGravity, Gravity.TOP | Gravity.LEFT);
        ta.recycle();
    }
    ...
    private void drawText(int size, float degrees, Canvas canvas, float textHeight, boolean isTop) {
        ...
        float textBaseY = size / 2f - (mTextPaint.descent() + mTextPaint.ascent()) / 2 + delta;
        canvas.drawText(mTextAllCaps ? mTextContent.toUpperCase() : mTextContent,
                getPaddingLeft() + (size - getPaddingLeft() - getPaddingRight()) / 2f, textBaseY, mTextPaint);
        canvas.restore();
    }
    ...
    private void drawTextWhenFill(int size, float degrees, Canvas canvas, boolean isTop) {
        canvas.save();
        canvas.rotate(degrees, size / 2f, size / 2f);
        float delta = isTop ? -size / 4f : size / 4f;
        float textBaseY = size / 2f - (mTextPaint.descent() + mTextPaint.ascent()) / 2 + delta;
        canvas.drawText(mTextAllCaps ? mTextContent.toUpperCase() : mTextContent,
                getPaddingLeft() + (size - getPaddingLeft() - getPaddingRight()) / 2f, textBaseY, mTextPaint);
        canvas.restore();
