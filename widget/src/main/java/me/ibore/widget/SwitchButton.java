...
public class SwitchButton extends View implements Checkable {
    ...
    private final int DEFAULT_WIDTH = UIUtils.dp2px(getContext(), 58);
    private final int DEFAULT_HEIGHT = UIUtils.dp2px(getContext(), 36);
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        shadowEffect = optBoolean(typedArray,
                R.styleable.SwitchButton_sbShadowEffect,
                true);

        uncheckCircleColor = optColor(typedArray,
                R.styleable.SwitchButton_sbUncheckCircleColor,
                0XffAAAAAA);//0XffAAAAAA;

        uncheckCircleWidth = optPixelSize(typedArray,
                R.styleable.SwitchButton_sbUncheckCircleWidth,
                UIUtils.dp2px(getContext(), 1.5f));//UIUtils.dp2px(getContext(), 1.5f);

        uncheckCircleOffsetX = UIUtils.dp2px(getContext(), 10);

        uncheckCircleRadius = optPixelSize(typedArray,
                R.styleable.SwitchButton_sbUncheckCircleRadius,
                UIUtils.dp2px(getContext(), 4));//UIUtils.dp2px(getContext(), 4);

        checkedLineOffsetX = UIUtils.dp2px(getContext(), 4);
        checkedLineOffsetY = UIUtils.dp2px(getContext(), 4);

        shadowRadius = optPixelSize(typedArray,
                R.styleable.SwitchButton_sbShadowRadius,
                UIUtils.dp2px(getContext(), 2.5f));//UIUtils.dp2px(getContext(), 2.5f);

        shadowOffset = optPixelSize(typedArray,
                R.styleable.SwitchButton_sbShadowOffset,
                UIUtils.dp2px(getContext(), 1.5f));//UIUtils.dp2px(getContext(), 1.5f);

        shadowColor = optColor(typedArray,
                R.styleable.SwitchButton_sbShadowColor,
                0X33000000);//0X33000000;

        uncheckColor = optColor(typedArray,
                R.styleable.SwitchButton_sbUncheckColor,
                0XffDDDDDD);//0XffDDDDDD;

        checkedColor = optColor(typedArray,
                R.styleable.SwitchButton_sbCheckedColor,
                0Xff51d367);//0Xff51d367;

        borderWidth = optPixelSize(typedArray,
                R.styleable.SwitchButton_sbBorderWidth,
                UIUtils.dp2px(getContext(), 1));//UIUtils.dp2px(getContext(), 1);

        checkLineColor = optColor(typedArray,
                R.styleable.SwitchButton_sbCheckLineColor,
                Color.WHITE);//Color.WHITE;

        checkLineWidth = optPixelSize(typedArray,
                R.styleable.SwitchButton_sbCheckLineWidth,
                UIUtils.dp2px(getContext(), 1f));//UIUtils.dp2px(getContext(), 1.0f);

        checkLineLength = UIUtils.dp2px(getContext(), 6);

        int buttonColor = optColor(typedArray,
                R.styleable.SwitchButton_sbButtonColor,
                Color.WHITE);//Color.WHITE;

        int effectDuration = optInt(typedArray,
                R.styleable.SwitchButton_sbEffectDuration,
                300);//300;

        isChecked = optBoolean(typedArray,
                R.styleable.SwitchButton_sbChecked,
                false);

        showIndicator = optBoolean(typedArray,
                R.styleable.SwitchButton_sbShowIndicator,
                true);

        background = optColor(typedArray,
                R.styleable.SwitchButton_sbBackground,
                Color.WHITE);//Color.WHITE;

        enableEffect = optBoolean(typedArray,
                R.styleable.SwitchButton_sbEnableEffect,
                true);
